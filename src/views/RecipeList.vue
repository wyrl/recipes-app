<template>
    <div id="bg" class="md:bg-[length:100%_auto] bg-cover" style="min-height: 100vh">
        <div class="flex flex-col p-4 relative">
            ...
            <h1 class="self-center bg-[#3b82f6bd] shadow text-white p-4 rounded mt-10 text-4xl font-bold">Your posted
                recipes</h1>

        </div>
        <div class="p-4">
            <router-link :to="{ name: 'recipe-new' }"
                class="bg-blue-500 py-2 px-4 rounded shadow text-white mb-6 inline-block">New</router-link>
            ...
            <div class="text-center" v-else>
                <h1 class="text-2xl text-center bg-white rounded shadow py-4 px-[40px] inline-block">
                    You have no posted here. Click a <router-link :to="{ name: 'recipe-new' }" class="text-blue-500">new
                        recipe</router-link>.
                </h1>
            </div>
        </div>
