...
<template>
  <header class="bg-blue-500 p-4 text-white text-center">
    <div class="flex relative items-center">
      ...
      <h1 class="text-2xl font-bold self-start text-left md:text-center absolute md:left-[calc(100%/2-85px/2)]">
        <router-link :to="{ name: 'home' }">Recipes</router-link><br>
      </h1>
      <div v-if="!loggedIn" class="flex ml-auto">
      ...
      <div v-else class="flex items-center">
        <DropdownButton :text="`${userLoggedIn.firstname} ${userLoggedIn.lastname}`">
          <router-link :to="{ name: 'recipe-new' }" class="text-gray-700 block px-4 py-2 text-sm" role="menuitem">Post a
            new recipe</router-link>
          <router-link :to="{ name: 'recipe-list' }" class="text-gray-700 block px-4 py-2 text-sm" role="menuitem">See
            posted recipes</router-link>
